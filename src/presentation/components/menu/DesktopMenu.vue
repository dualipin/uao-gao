...
const toggle = (index: number) => {
  indexToggle.value = indexToggle.value === index ? -1 : index
}
</script>
    ...
    <nav class="fixed flex h-full w-64 flex-col bg-gray-950 text-gray-50">
      <div class="mb-10 flex items-center justify-center bg-gray-900 py-4">
        <img
          ...
          class="h-20 rounded-2xl object-cover"
        />
      </div>
      ...
      <div
        class="mt-auto flex flex-wrap items-center justify-center gap-4 bg-gray-900 py-3"
      >
        <LogoutButton />
        <!-- <RouterLink
          to="/login"
        ...
        <img
          :src="logoTabscoop"
          alt="logo tabscoop"
          class="h-24 rounded-2xl object-cover"
        />
      </div>
    </nav>
  </aside>
